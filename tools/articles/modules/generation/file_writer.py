...
class FileWriter:
    ...
    def extract_category_and_filename(self, url_path: str, locale: str = 'en') -> tuple:
        """
        Extract category and filename from URL path for multi-language support.
    ...
    def save_article(
        ...
        try:
            ...
            # Step 2: Extract category and filename with locale
            category, filename, _ = self.extract_category_and_filename(article_info['url_path'], locale)

            # Step 3: Create full directory path with locale: src/content/{locale}/{category}/
            ...
            print(f"❌ Error saving {article_info['title']} [{locale}]: {str(e)}")
            self.stats['errors'] += 1
            return False
    ...
    def save_failed_article(
        ...
        existing_urls = {item['url_path'] for item in failed_list}
        if article_info['url_path'] not in existing_urls:
            # Add new failed article
            failed_entry = {
                'row_number': article_info.get('row_number', 0),
                'url_path': article_info['url_path'],
                'title': article_info['title'],
                'keyword': article_info['keyword'],
                'reference': article_info.get('reference', ''),
                'error': error_msg,
                'timestamp': timestamp,
                'retry_count': 0,
