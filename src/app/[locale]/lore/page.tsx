...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = locale === 'en' ? '/lore' : `/${locale}/lore`
  ...
  routing.locales.forEach(loc => {
    languages[loc] = loc === 'en' ? '/lore' : `/${loc}/lore`
  })
...
export default async function LorePage({ params }: PageProps) {
  ...
  const articles = await getArticlesList('lore', locale)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            <nav className="text-sm text-gray-400 mb-6">
              ...
              {' › '}
              <span className="text-white">{t('breadcrumbLore')}</span>
            </nav>
