...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = locale === 'en' ? '/platforms' : `/${locale}/platforms`
  ...
  routing.locales.forEach(loc => {
    languages[loc] = loc === 'en' ? '/platforms' : `/${loc}/platforms`
  })
...
export default async function PlatformsPage({ params }: PageProps) {
  ...
  const articles = await getArticlesList('platforms', locale)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            <nav className="text-sm text-gray-400 mb-6">
              ...
              {' › '}
              <span className="text-white">{t('breadcrumbPlatforms')}</span>
            </nav>
