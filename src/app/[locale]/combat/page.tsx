...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = locale === 'en' ? '/combat' : `/${locale}/combat`
  ...
  routing.locales.forEach(loc => {
    languages[loc] = loc === 'en' ? '/combat' : `/${loc}/combat`
  })
...
export default async function CombatPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations('combat')
  const tp = await getTranslations('pages')
  const articles = await getArticlesList('combat', locale)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            <nav className="text-sm text-gray-400 mb-6">
              ...
              {' › '}
              <span className="text-white">{t('breadcrumbCombat')}</span>
            </nav>
