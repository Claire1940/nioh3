...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = locale === 'en' ? '/world' : `/${locale}/world`
  ...
  routing.locales.forEach(loc => {
    languages[loc] = loc === 'en' ? '/world' : `/${loc}/world`
  })
...
export default async function WorldPage({ params }: PageProps) {
  ...
  const articles = await getArticlesList('world', locale)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            <nav className="text-sm text-gray-400 mb-6">
              ...
              {' › '}
              <span className="text-white">{t('breadcrumbWorld')}</span>
            </nav>
